...
const ManagerSlots = () => {
    ...
    const fetchSlots = async () => {
        if (!manager?.turf_id) return;
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE_URL}/api/slots/${manager.turf_id}?date=${date}`);
            setSlots(res.data);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const generateSlots = async () => {
        try {
            await axios.post(`${API_BASE_URL}/api/slots/generate`, {
    ...
    const updateSlot = async (id, data) => {
        try {
            await axios.put(`${API_BASE_URL}/api/slots/${id}`, data);
            fetchSlots();
        } catch (err) {
            alert('Update failed');
        }

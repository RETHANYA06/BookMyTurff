...
const TurfSearch = () => {
    ...
    const fetchTurfs = async () => {
        ...
        try {
            ...
            const res = await axios.get(`${API_BASE_URL}/api/turfs?${query}`);
            setTurfs(res.data);
        } catch (err) {
            console.error(err);
        } finally {
        ...
        <div className="fade-in" style={{ paddingBottom: '60px' }}>
            <header style={{ marginBottom: '60px', textAlign: 'center', paddingTop: '40px' }}>
                <div style={{ marginBottom: '20px' }}>
                    <span style={{ color: 'var(--primary)', fontWeight: '800', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '4px', background: 'rgba(16, 185, 129, 0.1)', padding: '8px 20px', borderRadius: '40px', border: '1px solid rgba(16, 185, 129, 0.2)' }}>Tactical Turf Scouting</span>
                </div>
                <h1 style={{ fontSize: '4rem', color: 'var(--text-primary)', fontWeight: '900', letterSpacing: '-2px', lineHeight: '1' }}>ELITE <span className="text-glow" style={{ color: 'var(--primary)' }}>TURF</span> FINDER</h1>
            ...
            <div className="glass-heavy" style={{ padding: '30px', borderRadius: '32px', marginBottom: '60px', display: 'flex', flexWrap: 'wrap', gap: '25px', alignItems: 'flex-end', position: 'sticky', top: '100px', zIndex: 100, border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
                <div style={{ flex: '1.5', minWidth: '200px' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', fontSize: '0.75rem', fontWeight: '800', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px' }}><FiSearch size={16} color="var(--primary)" /> Turf Name</label>
                    <input
                ...
                <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '30px' }}>
                    ...
                    {turfs.length === 0 && (
                        <div className="glass" style={{ gridColumn: '1/-1', textAlign: 'center', padding: '120px 40px', borderRadius: '40px', border: '1px solid rgba(0,0,0,0.05)' }}>
                            ...
                            <h2 style={{ fontSize: '2.5rem', color: 'var(--text-primary)', fontWeight: '900', letterSpacing: '-1px' }}>No Turf Detected</h2>

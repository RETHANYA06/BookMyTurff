...
const TurfDetails = () => {
    ...
    const fetchSlots = async () => {
        ...
        try {
            const res = await axios.get(`${API_BASE_URL}/api/slots/${id}?date=${date}`);
            setSlots(res.data);
        } finally {
    ...
    const toggleSlot = async (slot) => {
        ...
        if (isSelected) {
            try {
                await axios.post(`${API_BASE_URL}/api/slots/unlock`, { slotId: slot._id, playerId });
        ...
        try {
            const res = await axios.post(`${API_BASE_URL}/api/slots/lock`, { slotId: slot._id, playerId });
            ...
            if (selectedSlots.length === 0) {
                ...
                if (isContinuous) {
                    ...
                    await Promise.all(selectedSlots.map(s =>
                        axios.post(`${API_BASE_URL}/api/slots/unlock`, { slotId: s._id, playerId: player.id })
                    ));
                    setSelectedSlots([lockedSlot]);
                    setTimeLeft(180);
                }
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 420px', gap: '35px', alignItems: 'start' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '35px' }}>
                        ...
                        <section className="glass-heavy" style={{ padding: '0', borderRadius: '40px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 30px 60px rgba(0,0,0,0.4)' }}>
                            <div style={{ height: '420px', position: 'relative', backgroundColor: '#0f172a' }}>
                                <img
                            ...
                            <div style={{ padding: '40px 50px 50px', marginTop: '-60px', position: 'relative', background: 'var(--card-bg)', borderRadius: '40px 40px 0 0' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        <h1 style={{ fontSize: '3.5rem', marginBottom: '8px', color: 'var(--text-primary)', fontWeight: '900', letterSpacing: '-2.5px', lineHeight: '1.1' }}>{turf.turf_name}</h1>

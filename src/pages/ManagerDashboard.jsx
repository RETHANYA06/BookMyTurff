...
const ManagerDashboard = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [statsRes, bookingsRes] = await Promise.all([
                axios.get(`${API_BASE_URL}/api/bookings/earnings/${manager.turf_id}`),
                axios.get(`${API_BASE_URL}/api/bookings/${manager.turf_id}`)
            ]);
    ...
    const handleQuickAction = async (id, action, reason = '') => {
        ...
        try {
            if (action === 'complete') {
                await axios.post(`${API_BASE_URL}/api/bookings/${id}/complete`);
            } else if (action === 'confirm') {
                await axios.put(`${API_BASE_URL}/api/bookings/${id}`, { status: 'booked' });
            } else if (action === 'cancel') {
                await axios.put(`${API_BASE_URL}/api/bookings/${id}`, { status: 'cancelled', cancel_reason: reason });
            } else if (action === 'paid') {
                await axios.put(`${API_BASE_URL}/api/bookings/${id}`, { payment_status: 'fully_paid' });
            }
            fetchData();
        } catch (err) {
